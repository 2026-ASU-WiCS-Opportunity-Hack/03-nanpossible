...
- `/clients` includes the migrated wial.org/client-testimonials/ content as the first `testimonial_grid` section (a `ContentSection` type: quote/context/attribution cards, optional org logo from `public/clients/`, optional lazy `youtube-nocookie` video embed): quotes are verbatim with light trims, Lyreco's video + long case study are merged into one card, and Share Your Success Story was removed as unused (aliases `client-testimonials` → `/clients`, `share-your-success-story` → `/contact`); `scripts/sync-content-artifacts.mjs` also renders the type for seeded `body_html`
...
- `/certification` shows one expandable pathway section: four CALC→PALC→SALC→MALC cards with arrow separators; a card's `+` opens the level detail (below the row on desktop, inline on mobile); anchors `#calc`…`#malc`/`#progression` live on the cards; recertification is intentionally not rendered publicly (renewal data in `src/content/certification-hub.ts` kept for the chatbot)
