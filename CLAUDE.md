...
- `COHERE_API_KEY` is not set in Vercel (prod or preview), so coach approval skips embedding generation (`approved-search-off` notice); once the key is added, backfill missing vectors with `embedPendingApprovedCoaches` (`src/lib/coach-embeddings.ts`)
