...
- Admin country fields use the `CountrySelectField` dropdown (`src/components/country-select-field.tsx`), filtered by `listTakenCountries` (`src/lib/tenant.ts`) so a country with an existing affiliate can't be picked again (issue #74); the edited affiliate's own country (even legacy non-canonical names) stays selectable
