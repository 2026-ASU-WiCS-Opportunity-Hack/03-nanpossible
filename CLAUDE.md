...
- `/affiliates` cards use `contact_cards` with optional `image`/`imageAlt` (country-flag SVGs vendored in `public/flags/`, from flag-icons); `contact_cards` links starting with `http` open in a new tab; the affiliate list is static content — adding an affiliate means editing both `pages.json` and a `content_pages` migration with its website URL
