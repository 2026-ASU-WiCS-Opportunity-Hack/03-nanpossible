...
The Better World Fund donation form is migrated to `/better-world/donate` (#127): the donor picks a preset or enters a custom amount, and the server builds the Checkout Session's Price inline rather than reusing `/pay`'s fixed-price cards. The Better World Fund application form is migrated (`/better-world/nominate`, #123).
...
| Better World Fund donation form | `/better-world/donate` (Stripe Checkout, donor-chosen amount) | #127 |
| Homepage content | `/` | #9 |
